...
export async function sendContactNotification({ name, email, message, createdAt }) {
  try {
    const emailData = await resend.emails.send({
      from: 'Portfolio <noreply@nicolasvivaudou.com>', // Remplace par ton domaine
      to: [process.env.NOTIFICATION_EMAIL || 'contact@nicolasvivaudou.com'],
...
Voir dans l'admin: ${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/admin/contact
    ...
    console.log('✅ Email de notification envoyé:', emailData.id);
